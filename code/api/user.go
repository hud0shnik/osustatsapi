...
func parseBeatmapsString(pageStr string, left int) ([]BeatmapString, int) {
	...
	for index(pageStr, "{ artist", left) != -1 {
		...
		// Запись данных
		bm.Artist, left = findWithIndex(pageStr, "artist : ", " , artist_", left, end)
		bm.ArtistUnicode, left = findWithIndex(pageStr, "artist_unicode : ", " ,", left, end)

		bm.Covers.Cover, left = findWithIndex(pageStr, "cover : ", " , cover", left, end)
		bm.Covers.Cover = strings.ReplaceAll(bm.Covers.Cover, "\\", "")
		bm.Covers.Cover2X, left = findWithIndex(pageStr, "cover@2x : ", " ,", left, end)
		bm.Covers.Cover2X = strings.ReplaceAll(bm.Covers.Cover2X, "\\", "")
		bm.Covers.Card, left = findWithIndex(pageStr, "card : ", " , card@2x", left, end)
		bm.Covers.Card = strings.ReplaceAll(bm.Covers.Card, "\\", "")
		bm.Covers.Card2X, left = findWithIndex(pageStr, "card@2x : ", " ,", left, end)
		bm.Covers.Card2X = strings.ReplaceAll(bm.Covers.Card2X, "\\", "")
		bm.Covers.List, left = findWithIndex(pageStr, "list : ", " ,", left, end)
		bm.Covers.List = strings.ReplaceAll(bm.Covers.List, "\\", "")
		bm.Covers.List2X, left = findWithIndex(pageStr, "list@2x : ", " ,", left, end)
		bm.Covers.List2X = strings.ReplaceAll(bm.Covers.List2X, "\\", "")
		bm.Covers.SlimCover, left = findWithIndex(pageStr, "slimcover : ", " , slimcover", left, end)
		bm.Covers.SlimCover = strings.ReplaceAll(bm.Covers.SlimCover, "\\", "")
		bm.Covers.SlimCover2X, left = findWithIndex(pageStr, "slimcover@2x : ", " }", left, end)
		bm.Covers.SlimCover2X = strings.ReplaceAll(bm.Covers.SlimCover2X, "\\", "")

		bm.Creator, left = findWithIndex(pageStr, "creator : ", " ", left, end)
		bm.FavoriteCount, left = findWithIndex(pageStr, "favourite_count :", ",", left, end)
		bm.Hype, left = findWithIndex(pageStr, "hype :", ",", left, end)
		bm.Id, left = findWithIndex(pageStr, "id :", ",", left, end)
		bm.Nsfw, left = findWithIndex(pageStr, "nsfw :", ",", left, end)
		bm.Offset, left = findWithIndex(pageStr, "offset :", ",", left, end)
		bm.PlayCount, left = findWithIndex(pageStr, "play_count :", ",", left, end)
		bm.PreviewUrl, left = findWithIndex(pageStr, "preview_url : \\/\\/", " , source", left, end)
		bm.PreviewUrl = strings.ReplaceAll(bm.PreviewUrl, "\\", "")
		bm.Source, left = findWithIndex(pageStr, "source :", " ", left, end)
		bm.Spotlight, left = findWithIndex(pageStr, "spotlight :", ",", left, end)
		bm.Status, left = findWithIndex(pageStr, "status : ", " ,", left, end)
		bm.Title, left = findWithIndex(pageStr, "title : ", " , title_unicode", left, end)
		bm.TitleUnicode, left = findWithIndex(pageStr, "title_unicode : ", " ,", left, end)
		bm.TrackId, left = findWithIndex(pageStr, "track_id :", ",", left, end)
		bm.UserId, left = findWithIndex(pageStr, "user_id :", ",", left, end)
		bm.Video, left = findWithIndex(pageStr, "video :", ",", left, end)
		bm.DownloadDisabled, left = findWithIndex(pageStr, "download_disabled :", ",", left, end)

		bm.Bpm, left = findWithIndex(pageStr, "bpm :", ",", left, end)
		bm.CanBeHyped, left = findWithIndex(pageStr, "can_be_hyped :", ",", left, end)
		bm.DiscussionEnabled, left = findWithIndex(pageStr, "discussion_enabled :", ",", left, end)
		bm.DiscussionLocked, left = findWithIndex(pageStr, "discussion_locked :", ",", left, end)
		bm.IsScoreable, left = findWithIndex(pageStr, "is_scoreable :", ",", left, end)
		bm.LastUpdated, left = findWithIndex(pageStr, "last_updated : ", " ", left, end)
		bm.LegacyThreadUrl, left = findWithIndex(pageStr, "legacy_thread_url : ", " ", left, end)
		bm.LegacyThreadUrl = strings.ReplaceAll(bm.LegacyThreadUrl, "\\", "")

		bm.Nominations.Current, left = findWithIndex(pageStr, "current :", ",", left, end)
		bm.Nominations.Required, left = findWithIndex(pageStr, "required :", "}", left, end)

		bm.Ranked, left = findWithIndex(pageStr, "ranked :", ",", left, end)
		bm.RankedDate, left = findWithIndex(pageStr, "ranked_date : ", " ", left, end)
		bm.Storyboard, left = findWithIndex(pageStr, "storyboard :", ",", left, end)
		bm.SubmittedDate, left = findWithIndex(pageStr, "submitted_date : ", " ", left, end)
		bm.Tags = strings.Split(find(pageStr, "tags : ", " ,", left), " ")
		if bm.Tags[0] == "" {
			bm.Tags = nil
		}

		bm.Beatmap.BeatmapSetId, left = findWithIndex(pageStr, "beatmapset_id :", ",", left, end)
		bm.Beatmap.DifficultyRating, left = findWithIndex(pageStr, "difficulty_rating :", ",", left, end)
		bm.Beatmap.Id, left = findWithIndex(pageStr, "id :", ",", left, end)
		bm.Beatmap.Mode, left = findWithIndex(pageStr, "mode : ", " ", left, end)
		bm.Beatmap.Status, left = findWithIndex(pageStr, "status : ", " ", left, end)
		bm.Beatmap.TotalLength, left = findWithIndex(pageStr, "total_length :", ",", left, end)
		bm.Beatmap.UserId, left = findWithIndex(pageStr, " user_id :", ",", left, end)
		bm.Beatmap.Version, left = findWithIndex(pageStr, "version : ", " , accuracy", left, end)
		bm.Beatmap.Accuracy, left = findWithIndex(pageStr, "accuracy :", ",", left, end)
		bm.Beatmap.Ar, left = findWithIndex(pageStr, "ar :", ",", left, end)
		bm.Beatmap.Convert, left = findWithIndex(pageStr, "convert :", ",", left, end)
		bm.Beatmap.CountCircles, left = findWithIndex(pageStr, "count_circles :", ",", left, end)
		bm.Beatmap.CountSliders, left = findWithIndex(pageStr, "count_sliders :", ",", left, end)
		bm.Beatmap.CountSpinners, left = findWithIndex(pageStr, "count_spinners :", ",", left, end)
		bm.Beatmap.Cs, left = findWithIndex(pageStr, " cs :", ",", left, end)
		bm.Beatmap.DeletedAt, left = findWithIndex(pageStr, "deleted_at :", ",", left, end)
		bm.Beatmap.Drain, left = findWithIndex(pageStr, "drain :", ",", left, end)
		bm.Beatmap.HitLength, left = findWithIndex(pageStr, "hit_length :", ",", left, end)
		bm.Beatmap.IsScoreable, left = findWithIndex(pageStr, "is_scoreable :", ",", left, end)
		bm.Beatmap.LastUpdated, left = findWithIndex(pageStr, "last_updated : ", " ", left, end)
		bm.Beatmap.ModeInt, left = findWithIndex(pageStr, "mode_int :", ",", left, end)
		bm.Beatmap.PassCount, left = findWithIndex(pageStr, "passcount :", ",", left, end)
		bm.Beatmap.PlayCount, left = findWithIndex(pageStr, "playcount :", ",", left, end)
		bm.Beatmap.Ranked, left = findWithIndex(pageStr, "ranked :", ",", left, end)
		bm.Beatmap.Url, left = findWithIndex(pageStr, "url : ", " ", left, end)
		bm.Beatmap.Url = strings.ReplaceAll(bm.Beatmap.Url, "\\", "")
		bm.Beatmap.Checksum, left = findWithIndex(pageStr, "checksum : ", " ", left, end)
